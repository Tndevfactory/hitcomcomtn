...
<div class='container my-3'>
 <nav aria-label="breadcrumb">
    ...
    <div class='row row-cols-1 row-cols-md-2 product-details'>
        ...
        <img id='imgScreen' src="{{ app()->environment('production') ? asset(isset($product->images[0]->product_image) ? 'public/'.$product->images[0]->product_image : 'public/media/products/support.png') :
        ...
        <div class='col'>
            ...
            <div class='d-flex justify-content-between align-items-center'>
                    ...
                    <div>
                        ...
                        @if($product->discount !== '0.00')
                         <span class='text-capitalize d-inline fw-bold '> {{ __('discount') }}: <span class='text-success ms-1 ' dir='ltr'>
                         -{{ number_format($product->discount,0,'.',',')  }}% </span></span>
                         @else
                             {{ '' }}
                         @endif
                ...
                <div class='d-flex justify-content-between align-items-center'>
                    ...
                    <div>
                        ...
                        <span class=' text-uppercase '>
                                <span class=' {{ $product->status === 'available' ? 'text-success' : 'text-danger'}}  ms-1' style='font-size:0.8rem;font-weight:400'> {{ $product->status === 'available' ? 'en stock' : 'non disponible'}} </span>
                    ...
                    <div class=''>
                            ...
                            <span class="fs-6 ms-1 badge">
                             ...
                             @for ($i = 0; $i < 5; $i++)
                                @if (floor($product_rating->avg_rating_product) - $i >= 1)
                                    {{--Full Start--}}
                                    <span class="fa fa-star " style='font-size:1rem;color:#FF9529'></span>
                                @elseif ($product_rating->avg_rating_product - $i > 0)
                                    {{--Half Start--}}
                                ...
                                @endif
                            @endfor
                            <span class=" text-secondary fs-6 fw-light">({{$product_comment_count->comment_count }})</span> 
                            <span>
                    </div>
                </div>
            </li>
            ...
            <li class="list-group-item border border-0" style='font-size:0.9rem'>
                <div class='d-flex justify-content-between align-items-center '>
                      <div>
                        <span class='text-capitalize fw-bold'> {{ __('Seller:') }}</span>
                        <a href='{{ route('show-seller', [ 'shop_slug'=> $shop->shop_slug, 'currency' => Request()->session()->get('currency'), 'language' => App::getLocale()]) }}' 
                        class='text-decoration-none'><span class=' text-uppercase fw-bold  ms-1 text-secondary'>
